...
import { IronfinanceBalance, AutofarmBalance } from '../../models';
import AutofarmBalancesChart from './AutofarmBalancesChart';
import chainToColor from '../../utils/chainToColor.json';

const AutofarmDataFetcher = () => {
  ...
  const farmsSeries = map(groupedByFarm, (balances) => ({
    type: 'area',
      ...
      Number.parseFloat(
        (
          Object.values(groupedByFarm).reduce(
            (prev, current) =>
              prev +
              current.find((e) => e.createdAt.substring(0, 16) === item.createdAt.substring(0, 16))
                ?.balance,
            0
          ) - item.balance
        ).toFixed(2)
  ...
  return <AutofarmBalancesChart series={series} />;
};

export default AutofarmDataFetcher;
